...
class SSN:
    ...
    def __init__(self, data, outdir="./", log=None): 
        ...
        self.data = pd.read_csv(data, index_col=0, header=0,sep="\t") 
        self.csn = None
    ...
    @uf.robust
    def csnet(self, cells=None, alpha=0.01, boxsize=0.1, edgeW=0, to_csv=0, *args, **kwargs):
        """
        ...
        #每个new cell都要和原来所有的额细胞一起计算lower upper边界矩阵，都要排序每个基因来计算。
        ...
        #Construction of sample-specific networks for each cell 后续可用网络图论来研究样品间差异
        cells = self.get_cells(cells=cells)
        csn = dict()
        #dict.fromkeys(cells)
        
        B = pd.DataFrame(np.zeros((nr,nc)),columns=data.columns,index=data.index) #每个cell一个B_matrix，基因在box内都是1，不在是0
        p = -stats.norm.ppf(q=alpha,loc=0,scale=1) #0.99置信度下的统计量阈值 Percent point function (inverse of cdf — percentiles). 
    ...
    def csndm(self, cells=None, normalize=1, to_csv=1, nodeW=0, *args, **kwargs):
        ...
        for k in cells:
            ...
            if nodeW:
                ...
                ndm.loc[:,k] = csn[k].sum(axis=1) - np.diag(csn[k])
            celln += 1
            self.logger.info("Network degree matrix of cell {} is complete".format(k))
...
if __name__ == '__main__':                                                    
    ...
    csnet = csn.csnet(cells=args.netcells, 
